...
const md = require("markdown-it")({
  ...
  highlight: function (str, lang) {
    ...
    if (lang && hljs.getLanguage(lang)) {
      try {
        return '<pre class="nohighlight example"><code>' + hljs.highlight(str, { language: lang, ignoreIllegals: true }).value + "</code></pre>";
      } catch (__) {}
    }
...
async function getCodeSamples(data, mediatype) {
  ...
  if (op && op["x-code-samples"]) {
    ...
    const samplesGenerator = data.options.httpsnippet || ((mediatype || "").startsWith("multipart/") && !data.options.user_templates && data.options.experimental) ? httpsnippetGenerator.generate : fileTemplateGenerator;
