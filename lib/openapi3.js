  ...
  if (effSecurity && effSecurity.length && data.api.components && data.api.components.securitySchemes) {
    for (let ess of effSecurity) {
      if (data.api.components.securitySchemes[ess]) {
        ...
        if (!existingAuth && (secScheme.type === "oauth2" || secScheme.type === "openIdConnect" || (secScheme.type === "http" && (secScheme.scheme === "bearer" || secScheme.scheme === "basic")))) {
...
function convertInner(api, options) {
  return new Promise(function (resolve, reject) {
    ...
    data.version =
      data.api.info && data.api.info.version && typeof data.api.info.version === "string" && data.api.info.version.toLowerCase().startsWith("v") ? data.api.info.version : "v" + (data.api.info ? data.api.info.version : "v1.0.0");
...
function convert(api, options) {
  if (options.resolve) {
    return swagger2openapi
      .convertObj(api, { resolve: true, source: options.source, verbose: options.verbose })
      .then((sOptions) => {
